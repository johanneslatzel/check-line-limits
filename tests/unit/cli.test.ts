...
describe('CLI', () => {
    it('exits 0 when all files are within limits', () => {
        ...
        writeFileSync(
            ...
            JSON.stringify({ warn: 500, limit: 1000, scanDirs: ['src'], exclude: [] })
        );
        const { exitCode, stdout } = runCli([]);
        expect(exitCode).toBe(0);
        expect(stdout).toContain('within limits');
    });

    it('exits 1 when a file exceeds the limit', () => {
        mkdirSync(join(tmpDir, 'src'));
        writeFileSync(join(tmpDir, 'src/big.ts'), generateCode(20));
        writeFileSync(
            join(tmpDir, 'line-limits.json'),
            JSON.stringify({ warn: 5, limit: 10, scanDirs: ['src'], exclude: [] })
        );
        const { exitCode, stderr } = runCli([]);
        expect(exitCode).toBe(1);
        expect(stderr).toContain('SLOC (limit 10)');
    });
    ...
    it('outputs JSON with --json flag', () => {
        ...
        expect(parsed).toHaveProperty('hasOverLimit', false);
        expect(parsed).toHaveProperty('warn', 500);
        expect(parsed).toHaveProperty('limit', 1000);
    });

    it('respects --limit override', () => {
        mkdirSync(join(tmpDir, 'src'));
