...
describe('CLI integration', () => {
    ...
    it('outputs JSON when --json flag is used', () => {
        ...
        expect(output).toHaveProperty('hasOverLimit');
    ...
    it('respects custom config file', () => {
        ...
        writeFileSync(join(tmpDir, 'custom.json'), JSON.stringify({ warn: 10, limit: 20 }));

        // Create a file that exceeds custom limit
        mkdirSync(join(tmpDir, 'src'), { recursive: true });
    ...
    it('respects --warn and --limit overrides', () => {
        mkdirSync(join(tmpDir, 'src'), { recursive: true });
        ...
        const result2 = spawnSync('node', [cliPath, '--limit', '10'], {
            cwd: tmpDir,
            encoding: 'utf8'
        });
    ...
    it('warns files between warn and limit thresholds', () => {
        mkdirSync(join(tmpDir, 'src'), { recursive: true });
