...
- [`walker.ts`](https://github.com/johanneslatzel/check-line-limits/blob/main/src/walker.ts): walks each scan directory recursively with `readdirSync`; prunes directories named in `skipDirs` (default `node_modules`, `.git`, `dist`, `coverage`); keeps only `.ts`, `.tsx`, `.js`, `.jsx`, and `.mjs` files not matched by an exclude pattern.
